...
function findAllPaths(graph, path, startVertex, endVertex, index) {
...
function isCycleExistsUitls(graph, startVertex, visited, recursion) {
    visited[startVertex] = true;
...
function isCyclic(graph) {
    ...
    let recusion = Array.from({ length: graph.vertex }, () => false);
...
function topologicalSort(graph, startVertex, visited, stack, pointer) {
    visited[startVertex] = true;
        ...
        switch (choice) {
            ...
            case 5:
                ...
                for (let i = 0; i < graph.vertex; i++) {
                    if (!visited[i]) {
                        topologicalSort(graph, i, visited, stack, pointer);
                    }
                }
