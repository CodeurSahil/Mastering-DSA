...
def findAllPaths(graph, path, startVertex, endVertex, vertex, index):
    ...
    for neighbour in range(0, vertex):
        ...
        if (value == 1):
            findAllPaths(graph, path, neighbour, endVertex, vertex, index);

    return;

def isCycleExistsUtils(graph, startVertex, visited, recursion, vertex):
    visited[startVertex] = True;
    ...
    for neighbour in range(0, vertex):
        ...
        if (value == 1 and recursion[neighbour]):
            return True;
        elif (value == 1 and (not visited[neighbour]) and isCycleExistsUtils(graph, neighbour, visited, recursion, vertex)):
            return True;
...
def isCycleExists(graph, vertex):
    ...
    for i in range(0, vertex):
        recursion = [False for _ in range(vertex)];
...
def topologicalSort(graph, startVertex, visited, stack, vertex):
    if (visited[startVertex]):
        return;
...
while True:
    ...
    if(choice == 1):
        ...
        findAllPaths(graph, path, startVertex, endVertex, vertex, index);
