...
function enQueue(queue) {
    ...
    while (queue.secondaryStack.top != -1) {
        queue.primaryStack.data.push(queue.secondaryStack.data.pop());
        queue.primaryStack.top++;
        queue.secondaryStack.top--;
    }
...
function deQueue(queue) {
    ...
    let removedVal = queue.primaryStack.data.pop();
    queue.primaryStack.top--;
        ...
        switch (choice) {
            ...
            case 3:
                if (queue.primaryStack.top == -1) {
                    ...
                    console.log(`\nData Element:- ${queue.primaryStack.data[queue.primaryStack.top]}\n`);
                }
                break;
            case 4:
                if (queue.primaryStack.top == -1) {
